...
final class CarTransformer
{
    ...
    public function transform(\WP_Post $post, string $locale = 'ko'): array
    {
        ...
        return [
            ...
            'carYear' => $this->meta($post->ID, 'car_year'),
            'carMake' => $this->meta($post->ID, 'car_make'),
            'carModel' => $this->meta($post->ID, 'car_model'),
            'mileageLabel' => $this->nullable($this->localized($post->ID, 'mileage_label', $locale)),
            'carLocation' => $this->nullable($this->localized($post->ID, 'car_location', $locale)),
