...
final class LoanTransformer
{
    ...
    public function transform(\WP_Post $post, string $locale = 'ko'): array
    {
        ...
        return [
            ...
            'loanType' => $this->meta($post->ID, 'loan_type'),
            'interestRateLabel' => $this->nullable($this->localized($post->ID, 'interest_rate_label', $locale)),
            'loanAmountLabel' => $this->nullable($this->localized($post->ID, 'loan_amount_label', $locale)),
            'location' => $this->nullable($this->localized($post->ID, 'location', $locale)),
