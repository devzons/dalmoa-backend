...
final class MarketplaceTransformer
{
    ...
    public function transform(\WP_Post $post, string $locale = 'ko'): array
    {
        ...
        return [
            ...
            'itemCondition' => $this->enumLabel('item_condition', $post->ID, $locale),
            'itemLocation' => $this->nullable($this->localized($post->ID, 'item_location', $locale)),
    ...
    private function enumLabel(string $key, int $postId, string $locale): ?string
    {
        ...
        return match ($key) {
            'item_condition' => DisplayLabel::itemCondition($value, $locale),
            default => $value,
        };
    }
}