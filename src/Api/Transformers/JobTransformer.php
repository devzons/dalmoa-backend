...
final class JobTransformer
{
    ...
    public function transform(\WP_Post $post, string $locale = 'ko'): array
    {
        ...
        return [
            ...
            'companyName' => $this->meta($post->ID, 'company_name'),
            'jobLocation' => $this->nullable($this->localized($post->ID, 'job_location', $locale)),
            'employmentType' => $this->enumLabel('employment_type', $post->ID, $locale),
            'salaryLabel' => $this->nullable($this->localized($post->ID, 'salary_label', $locale)),
            'contactEmail' => $this->meta($post->ID, 'contact_email'),
            'contactPhone' => $this->meta($post->ID, 'contact_phone'),
            'applyUrl' => $this->meta($post->ID, 'apply_url'),
